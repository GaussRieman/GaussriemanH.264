...
void recordDecFrameIntoYUVFile(char *filename, x264_frame_t *frame) {
	//记录图像到文件中
	FILE * pRecordFile;
	int ii;
	...
	else
	{
		...
		unsigned char *p = frame->plane[0];
		for (ii = 0; ii<frame->i_lines[0] - 8; ii++)
		{
			fwrite(p, 1, frame->i_width[0], pRecordFile);
			p += frame->i_stride[0];
		}

		p = frame->plane[1];
		for (ii = 0; ii<frame->i_lines[1] - 8; ii++)
		{
			fwrite(p, 1, frame->i_width[1], pRecordFile);
			p += frame->i_stride[1];
		}

		p = frame->plane[2];
		for (ii = 0; ii<frame->i_lines[2] - 8; ii++)
		{
			fwrite(p, 1, frame->i_width[2], pRecordFile);
			p += frame->i_stride[2];
		}
...
#define BS_BAK_ROW_VBV        3
	...
	//在码流中预留写文字块length的空间
	bs_align_1(&h->out.bs); 
	uint32_t start = h->out.bs.p - h->out.bs.p_start;
	bs_write32(&h->out.bs,(uint32_t)0);
    struct TextEncoderContext *text_encoder_context;
    initialTextEncoderContext((1920+32)*(1080+32)/16/16, &text_encoder_context);
	int text_block_num = 0;

#ifdef OUTPUT_MBINFO
	FILE *fp_log;
	char log_name[1024];
	sprintf(log_name, "MBInfo_%d.log", h->fenc->i_frame);
	fp_log = fopen(log_name, "w");

	FILE *fp_frame_log;
	sprintf(log_name, "FramesInfo.log");
	if(h->fenc->i_frame==0)
		fp_frame_log = fopen(log_name, "w");
	else
		fp_frame_log = fopen(log_name, "ab+");
#endif

	//在此处增加全局运动向量的运算
	//Initialize the context of recognition
	struct RecognizeContext *recognize_context = NULL;
	initialRecognizeContext(h->param.i_width, h->param.i_height, h->param.i_width, h->param.i_height, kOneBitThreshold, &recognize_context);

	struct Frame *p_ref_frame = NULL;		//参考帧
	struct Frame *p_current_frame = NULL;	//当前帧
	allocAndSetFrame(h->param.i_width, h->param.i_height, h->fenc->i_frame, &p_current_frame);
	p_current_frame->stride = h->fenc->i_stride[0];
	p_current_frame->data_component1 = h->fenc->plane[0];
	p_current_frame->data_component2 = h->fenc->plane[1];
	if (CHROMA444)
		p_current_frame->data_component3 = h->fenc->plane[2];
	else
		//4:2:0由于X264数据排列为 YYYYYYYYYYYYYYYYYYYY, UVUVUVUVUVUVUVUVUVUVUV,不是分开的，所以暂时不支持
		p_current_frame->data_component3 = h->fenc->plane[1] + (p_current_frame->stride >> 1)*(h->param.i_height >> 1);


#ifdef GMV_ENABLE
	allocAndSetFrame(h->param.i_width, h->param.i_height, h->fenc->i_frame - 1, &p_ref_frame);
	readFrameFromYUVFile("D:\\work\\test_sequence\\app_JDshopping_1080p_30fps_444_295.yuv", p_ref_frame, h->fenc->i_frame - 1);
	p_ref_frame->c_frame_id = h->fenc->i_frame;

	generateMotionDetect(p_current_frame, p_ref_frame, recognize_context);
#endif

#ifdef IBC_ENABLE
	detectIntraCopyBlock(p_current_frame, recognize_context);
#endif

	//	printf("frame_id : %d, lbc_count: %d, ubc_count: %d, mvmatched_count: %d\n", h->fenc->i_frame,
	//			recognize_context->lbc_count, recognize_context->ubc_count, recognize_context->mvmatched_count);
    ...
    while( 1 )
    {
		...
		//保留bitstream原始状态
		uint8_t *mb_start_p = h->out.bs.p;
		uintptr_t mb_start_cur_bits  = h->out.bs.cur_bits;
		int     mb_start_i_left = h->out.bs.i_left;
		int     mb_start_i_bits_encoded = h->out.bs.i_bits_encoded;
...
//处理全局运动向量
		int cur_mb = i_mb_x + i_mb_y*recognize_context->mb_stride;
		if (recognize_context->mbtype[cur_mb] == MVMATCHED)
		{
			x264_macroblock_analyse(h, MVMATCHED, recognize_context->global_mv.x, recognize_context->global_mv.y);
		}
		else
		{
			x264_macroblock_analyse(h, 0, 0, 0);
		}

//处理IBC块
#ifdef IBC_ENABLE
		if( IS_INTRA(h->mb.i_type) && (recognize_context->mbtype[cur_mb] == LBC || recognize_context->mbtype[cur_mb] == UBC) )
		{
			if (recognize_context->mbtype[cur_mb] == LBC)
				h->mb.i_type = V_LBC;
			else
				h->mb.i_type = V_UBC;
			...
			//仿照p_skip类型，x264_analyse_update_cache
...
//				h->mc.copy[PIXEL_16x16](h->mb.pic.p_fdec[p], FDEC_STRIDE, p_source[p], FENC_STRIDE, 16);
        ...
        else
        {
            ...
            else
            {
                if( h->sh.i_type != SLICE_TYPE_I )
                {
                    ...
                    i_skip = 0;
					//保留bitstream原始状态
					mb_start_p = h->out.bs.p;
					mb_start_cur_bits = h->out.bs.cur_bits;
					mb_start_i_left = h->out.bs.i_left;
					mb_start_i_bits_encoded = h->out.bs.i_bits_encoded;
				}
		...
		//在X264编码结束后, 判断是否要采用文字编码
		pixel *p_source[3];
		p_source[0] = h->mb.pic.p_fenc[0];
		p_source[1] = h->mb.pic.p_fenc[1];
		p_source[2] = h->mb.pic.p_fenc[2];
#ifdef TEXT_ENABLE
...
#endif
		h->fdec->i_text_type[mb_xy] = h->mb.i_type;

#ifdef OUTPUT_MBINFO
		fprintf(fp_log, "\n i_mb_x: %d   i_mb_y: %d, mb_type: %d, total_bits:%d, mb_size: %d;", i_mb_x, i_mb_y, h->mb.i_type, total_bits, mb_size);
#endif
...
//	recordDecFrameIntoYUVFile("recons_frames.yuv",h->fdec);	
...
#ifdef OUTPUT_MBINFO
	int total_bits = bs_pos(&h->out.bs) + x264_cabac_pos(&h->cabac);
//	printf("\nframe=%d, text block count=%d, total bit=%d      |", h->fenc->i_frame, text_block_num, total_bits);
	
	fprintf(fp_frame_log, "\n frame=%d, text block count=%d, total bit=%d \n", h->fenc->i_frame, text_block_num, total_bits);
	fprintf( fp_log, "\n text block count=%d; total bit=%d  ", text_block_num, total_bits);
#endif
    if( x264_nal_end( h ) )
        return -1;
	...
	freeFrame(p_ref_frame, true);
	freeFrame(p_current_frame, false);		//不释放各个分量的指针
	freeRecognizeContext(recognize_context);
	releaseTextEncoderContext(text_encoder_context);
	return 0;
}
